...
contract SupplyChain {
    ...
    modifier verifyCaller(address _address) {
        require(msg.sender == _address, "caller is not the seller");
        _;
    }

    modifier paidEnough(uint256 _price) {
        require(msg.value >= _price, "value lower than price");
        _;
    }
    ...
    modifier sold(uint256 sku) {
        require(items[sku].price != 0 && items[sku].state == State.Sold);
        _;
    }
    modifier shipped(uint256 sku) {
        require(items[sku].price != 0 && items[sku].state == State.Shipped);
        _;
    }
    modifier received(uint256 sku) {
        require(items[sku].price != 0 && items[sku].state == State.Received);
        _;
    }
